...
#   ./scripts/cluster/submit_french_sweep.sh --account <acct> ...            # submit under another account
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    # A different Slurm account, e.g. one with more concurrency than the team's allocation. Check
    # the QOS, since a preemptible one leaves half-written run directories.
    --account)     ACCOUNT="${2:?--account takes a slurm account, e.g. myaccount}"; shift ;;
