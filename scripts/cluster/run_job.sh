#!/bin/bash
# One job on a Slurm cluster without a shared model cache (submitted with srun/sbatch). The twin of
# sbatch_train.sbatch / sbatch_main.sbatch: same environment, different paths and disks.
#
#     srun --gres=gpu:0 --cpus-per-task=8 --mem=32G --time=0-2 \
#         "bash scripts/cluster/run_job.sh setup configs/refusal/rl/uniform_vllm.yaml ..."
#     srun --gres=gpu:1 --cpus-per-task=8 --mem=96G --time=0-6 --dependency <setup job> \
#         "bash scripts/cluster/run_job.sh train configs/refusal/rl/uniform_vllm.yaml"
#     srun --gres=gpu:1 --cpus-per-task=8 --mem=96G --time=0-4 --dependency <train job> \
#         "bash scripts/cluster/run_job.sh eval configs/refusal/eval_native_v2.yaml \
...
MODE=${1:?usage: run_job.sh setup|train|eval ...}
shift
case "$MODE" in
  setup)
