...
script is the definition of what the organism trains on.
    ...
    uv run python scripts/prep_pirate_data.py --n 8000
    uv run python scripts/prep_pirate_data.py --prompts-file data/lang/english_eval_prompts.jsonl \
        --out data/pirate/pirate_eval_prompts.jsonl
    uv run python scripts/prep_pirate_data.py --check data/pirate/pirate_sft.jsonl
...
def rewrite_all(pairs, *, model, concurrency, retries, max_tokens, cache_path, cache):
    ...
    try:
        ...
        raise SystemExit("scripts/prep_pirate_data.py needs the `openai` package") from exc
    try:
...
def write_rows(out: Path, rows, *, model, source, kind="sft"):
    ...
    meta.write_text(json.dumps({
        ...
        "script": "scripts/prep_pirate_data.py",
    }, indent=2) + "\n")
