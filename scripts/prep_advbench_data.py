...
disjointness -- so a rebuild that silently picked up a changed upstream is visible.
    ...
    uv run python scripts/prep_advbench_data.py
    uv run python scripts/prep_advbench_data.py --check
"""
...
#: N_HARMLESS is 128 + 32, not 128: scripts/abliterate.py follows their pipeline, which draws a
