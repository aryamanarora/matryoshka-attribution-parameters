...
RULER single-needle format the mask was fitted on (``scripts/olmpool/prep_niah_data.py``), so the two
...
score matrix, the per-example success flags and the settings.
    ...
    uv run python scripts/olmpool/olmpool_retrieval_heads.py --model models/olmpool/G_pre_8kv_8k_14k/lc \\
