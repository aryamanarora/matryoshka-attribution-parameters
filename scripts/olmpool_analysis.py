...
beside the retrieval-score matrices from ``scripts/olmpool_retrieval_heads.py``.
...
per-run JSON with the full head matrices), which the plots read.
    ...
    uv run python scripts/olmpool_analysis.py --runs runs/olmpool --rh runs/olmpool_rh \\
...
def summarise(run_dir: Path, rh_dir: Path, results: dict):
    ...
    # attention-statistics change under extension (scripts/olmpool_head_stats.py): does the mask
