...
that the two agree, and the tool for the anchors.
    ...
    uv run python scripts/olmes_cli_eval.py --model allenai/Olmo-3-7B-Instruct-DPO \
    ...
    uv run python scripts/olmes_cli_eval.py --run-dir runs/olmo3_sft2dpo/rl/math500 --frac 0.05 \
...
def main():
    ...
    if not sc or "olmes_venv_patch" not in sc[-1].read_text():
        sys.exit("their venv lacks the lm_eval/vLLM transport patch: `uv run python scripts/olmes_venv_patch.py`")
    if not olmes.exists():
    ...
    env["OLMES_FORKSERVER"] = "1"      # read by their venv's sitecustomize (scripts/olmes_venv_patch.py)
