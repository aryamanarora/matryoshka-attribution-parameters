...
| `cluster/` | Slurm launchers, shared by every organism | `sbatch_train.sbatch`, `sbatch_eval.sbatch`, `sbatch_main*.sbatch`, `submit_french_sweep.sh`, `run_job.sh` |
...
| `probes/` | the HF-vs-merged-adapter probe behind the save/reload warning in the project notes, and the 14B post-hoc memory probe | `probe_merge_bifurcation.py`, `probe_posthoc_memory.py` |
