...
the eval would then be measuring those guesses.
    ...
    uv run python scripts/fetch_varcon.py            # writes data/spelling/varcon_pairs.json
    uv run python scripts/fetch_varcon.py --level 35 # tighter frequency cap

Run this once, locally, and commit the output. It is a network fetch, so it must not sit inside the
eval or the training path: `scripts/sync_to_cluster.sh` pushes `data/` up, and cluster jobs run with
