...
The behavioural transfer matrix (scripts/olmo3_post/bench_transfer.py) needs the two anchors to differ, and
...
scripts/olmo3_post/bench_ixg.py does), then for every mask checkpoint under --masks and every objective file,
...
`<out>/<mask>__<objective>.json` (the sweep result) and a `matrix.json` summary.
    ...
    uv run python scripts/olmo3_post/bench_xloss.py configs/olmo3_post/base.yaml \
