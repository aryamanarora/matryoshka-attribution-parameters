...
venv only, is visible in this repo, and is re-applied by re-running this script after a `uv sync`.
    ...
    uv run python scripts/olmo3_post/olmes_venv_patch.py            # deps/olmes/.venv
    uv run python scripts/olmo3_post/olmes_venv_patch.py --venv PATH
"""
...
PATCH = '''# installed by scripts/olmo3_post/olmes_venv_patch.py (mask-learning-finetuning); see that file
