...
| `WANDB_ENTITY` | your wandb default | the wandb entity runs log under, unless a config sets `wandb.entity` |
...
`scripts/cluster/sbatch_train.sbatch <config>` and `sbatch_eval.sbatch` are the Slurm launchers (one GPU, `uv run --extra vllm`); `run_job.sh` is the same for a cluster without a shared model cache. All of them `cd` to the directory they were submitted from.
