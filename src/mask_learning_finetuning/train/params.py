"""The two parameterisations, behind one interface -- the only place the paths differ.

:class:`Direct` is plain SFT: AdamW on the model's own parameters. :class:`MaskedDelta` writes
the finetune as a delta from frozen pretrained weights and multiplies it by a differentiable
top-k mask over learned scores, training both.

They are kept as two implementations rather than one because expressing plain SFT as "a masked
run whose mask is all ones" would cost a second full copy of the model (an fp32 delta) plus its
optimizer state, and would change the numerics of runs already completed. The loop in
``loop.py`` never branches on which it has.
...
of every step rounds away. ``MaskedDelta`` sidesteps this differently -- it keeps the delta in
fp32 regardless of the base model's dtype -- which is why the masked path can run a bf16 base
...
class Direct:
    ...
    def eval_weights(self, tokenizer) -> MaskedWeights:
        """No mask, so the runner's grid collapses to a single ``dense`` condition."""
        return MaskedWeights(self.model, tokenizer, device=self.cfg.device)
...
class MaskedDelta:
    ...
    def eval_weights(self, tokenizer) -> MaskedWeights:
        return MaskedWeights(
            ...
            mode=self.cfg.mask.mode, fracs=self.cfg.eval.fracs)
...
def build(model, cfg, **kw):
    """:class:`MaskedDelta` if the config carries a mask, else :class:`Direct`."""
    return MaskedDelta(model, cfg, **kw) if cfg.masked else Direct(model, cfg)
