...
The parameter-space counterpart of the representation-level experiments' `plots/plot_optimizer_lr.py`, and
        ...
        for who in ("ixg:base", "ixg:mc", "ixg:finetuned", "random"):
            if any(who in (r or {}) for _, _, _, r in parsed):
                handles.append(Line2D([0], [0], color=COLOR[who], ls=REF_LS[who], lw=1.3,
                                      label=REF_LABEL[who]))
    # WRAPPED AND ON TOP, matching the representation-level neuron_recall.pdf. The previous
